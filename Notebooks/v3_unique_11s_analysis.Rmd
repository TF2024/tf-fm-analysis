---
title: "v3 coaching attendance analysis"
...
# Join with follow-up status
status_11s <- all_records_with_y12q1 %>%
  select(upn, followup_period, status) %>%
  inner_join(all_prog_years_11s, by = "upn")

status_11s
n_distinct(status_11s$upn)

# Descriptives for 1:1s - all years
counts_11s_stats <- status_11s %>%
  group_by(prog_year) %>%
  summarise(
    total_records = n(),
